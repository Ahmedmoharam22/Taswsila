...
/**
 * كومبوننت اللوجو الموحد لمشروع "توصيلة".
 *
 * @param {string} size - حجم اللوجو (small, medium, large). الافتراضي medium.
 * @param {string} color - لون اللوجو والكلمة (brand, dark, light). الافتراضي brand.
 * @param {boolean} withText - هل يتم عرض كلمة "توصيلة" بالعربي؟ الافتراضي true.
 * @param {string} className - كلاسات إضافية للتخصيص من الخارج (للـ Positioning مثلاً).
 */
export const Logo = ({ 
  size = 'medium', 
  color = 'brand', 
  ...
  // 1. تحديد مقاسات الأيقونة والخط بناءً على الـ props
  const sizes = {
    small: { svg: { w: 24, h: 20 }, text: 'text-xl' },
    medium: { svg: { w: 32, h: 26 }, text: 'text-3xl' }, // المقاس الافتراضي
    large: { svg: { w: 49, h: 40 }, text: 'text-5xl' },  
  };

  const currentSize = sizes[size] || sizes.medium;

  // 2. تحديد الألوان (بناءً على الـ Config بتاع Tailwind بتاعنا)
  const colors = {
    brand: 'text-brand-600', // اللون الأزرق الأساسي للوجو
    dark: 'text-slate-900',  // لون غامق للـ Footer مثلاً
    light: 'text-white',     // لون فاتح للخلفيات الغامقة
  };

  const currentColor = colors[color] || colors.brand;

  return (
    <div className={`flex items-center gap-3 font-arabic font-extrabold ${currentColor} ${className}`}>
      
      {/* 3. الـ SVG اللوجو اللي إنت اخترته */}
      {/* استخدمنا fill="currentColor" عشان نتحكم في لونه بالـ Tailwind */}
      <svg 
        xmlns="http://www.w3.org/2000/svg" 
        width={currentSize.svg.w} 
        height={currentSize.svg.h} 
        viewBox="0 0 49 40" 
        fill="none" 
        id="Logo"
        className="transition-transform duration-300 hover:scale-110" // حركة صايعة لما تقف عليه
      >
        <g id="logomark">
          <path 
            fillRule="evenodd" 
            ...
            fill="currentColor" // أهم حاجة عشان نتحكم في اللون بالـ Tailwind
          />
        </g>
      </svg>

      {/* 4. كلمة "توصيلة" عربي بـ Font محترم */}
      {withText && (
